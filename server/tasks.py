...
# FIX: Changed from gpt-4-turbo to gpt-4o for proper vision/image support
...
def summarise_chunks(chunks, document_id, source_type="file"):
    ...
    for i, chunk in enumerate(chunks):
        ...
        if content_data['tables'] or content_data['images']:
            ...
            # FIX: Fallback to plain text if AI summary fails or returns None
            if not enhanced_content:
                print(f"     ⚠️ AI summary returned None, falling back to plain text")
                enhanced_content = content_data['text']
        else:
            enhanced_content = content_data['text']

        # FIX: Final safety fallback — ensure enhanced_content is never None or empty
        if not enhanced_content:
            enhanced_content = ""
        ...
        processed_chunk = {
            ...
            'char_count': len(enhanced_content)  # Safe now — enhanced_content is always a string
        }
...
SEARCH INDEX:"""
        ...
        return None  # Explicitly return None so caller can handle it
...
def store_chunks_with_embeddings(document_id: str, processed_chunks: list):
    ...
    # FIX: Ensure content is always a string, never None
    texts = [chunk_data['content'] or '' for chunk_data in processed_chunks]
